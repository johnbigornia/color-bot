import random
import time

from utilities import ocr
import utilities.api.item_ids as ids
import utilities.color as clr
import utilities.random_util as rd
from model.osrs.osrs_bot import OSRSBot
from utilities.api.morg_http_client import MorgHTTPSocket
from utilities.api.status_socket import StatusSocket
import utilities.imagesearch as imsearch


class Enchant_Bracelet(OSRSBot):
    def __init__(self):
        bot_title = "Enchant Bracelet"
        description = "enchant"
        super().__init__(bot_title=bot_title, description=description)
        # Set option variables below (initial value is only used during headless testing)
        self.running_time = 1
        self.api_m = MorgHTTPSocket()
        self.ENCHANT_IMAGES = imsearch.BOT_IMAGES.joinpath("for_scripts", "enchant_bracelet")
        self.diamond_enchant = None

    def create_options(self):
        """
        Use the OptionsBuilder to define the options for the bot. For each function call below,
        we define the type of option we want to create, its key, a label for the option that the user will
        see, and the possible values the user can select. The key is used in the save_options function to
        unpack the dictionary of options after the user has selected them.
        """
        self.options_builder.add_slider_option("running_time", "How long to run (minutes)?", 1, 500)

    def save_options(self, options: dict):
        """
        For each option in the dictionary, if it is an expected option, save the value as a property of the bot.
        If any unexpected options are found, log a warning. If an option is missing, set the options_set flag to
        False.
        """
        for option in options:
            if option == "running_time":
                self.running_time = options[option]
            elif option == "text_edit_example":
                self.log_msg(f"Text edit example: {options[option]}")
            elif option == "multi_select_example":
                self.log_msg(f"Multi-select example: {options[option]}")
            elif option == "menu_example":
                self.log_msg(f"Menu example: {options[option]}")
            else:
                self.log_msg(f"Unknown option: {option}")
                print("Developer: ensure that the option keys are correct, and that options are being unpacked correctly.")
                self.options_set = False
                return
        self.log_msg(f"Running time: {self.running_time} minutes.")
        self.log_msg("Options set successfully.")
        self.options_set = True

    def main_loop(self):
        """
        When implementing this function, you have the following responsibilities:
        1. If you need to halt the bot from within this function, call `self.stop()`. You'll want to do this
           when the bot has made a mistake, gets stuck, or a condition is met that requires the bot to stop.
        2. Frequently call self.update_progress() and self.log_msg() to send information to the UI.
        3. At the end of the main loop, make sure to call `self.stop()`.

        Additional notes:
        - Make use of Bot/RuneLiteBot member functions. There are many functions to simplify various actions.
          Visit the Wiki for more.
        - Using the available APIs is highly recommended. Some of all of the API tools may be unavailable for
          select private servers. For usage, uncomment the `api_m` and/or `api_s` lines below, and use the `.`
          operator to access their functions.
        """
        # Setup APIs
        # api_m = MorgHTTPSocket()
        # api_s = StatusSocket()

        # Main loop
        start_time = time.time()
        end_time = self.running_time * 60

        image_path2 = self.ENCHANT_IMAGES.joinpath("enchant_diamond.png")
        self.diamond_enchant = imsearch.search_img_in_rect(image_path2, self.win.control_panel, confidence=0.10)

        while time.time() - start_time < end_time:
            # -- Perform bot actions here -- 11095 enchanted 11092 not
            # Code within this block will LOOP until the bot is stopped.

            if self.api_m.get_if_item_in_inv(ids.DIAMOND_BRACELET):
                image_path = self.ENCHANT_IMAGES.joinpath("mage_book.png")
                mage_book = imsearch.search_img_in_rect(image_path, self.win.control_panel, confidence=0.10)
                if mage_book is None:
                    self.mouse.move_to(self.win.cp_tabs[6].random_point())
                    self.mouse.click()

                if random.random() < .85:
                    print("Fast enchanft")
                    for i in range(1, 28):
                        image_path = self.ENCHANT_IMAGES.joinpath("mage_book.png")
                        mage_book = imsearch.search_img_in_rect(image_path, self.win.control_panel, confidence=0.15)

                        image_path2 = self.ENCHANT_IMAGES.joinpath("enchant_diamond.png")
                        enchant = imsearch.search_img_in_rect(image_path2, self.win.control_panel, confidence=0.15)
                        waiting = True
                        while waiting:
                            print("waiting for mage book")
                            mage_book = imsearch.search_img_in_rect(image_path, self.win.control_panel, confidence=0.15)
                            enchant = imsearch.search_img_in_rect(image_path2, self.win.control_panel, confidence=0.15)
                            if enchant is not None or mage_book is not None:
                                waiting = False
                            if self.api_m.get_if_item_in_inv(ids.DIAMOND_BRACELET) is False:
                                break
                            time.sleep(.5)
                        if self.api_m.get_if_item_in_inv(ids.DIAMOND_BRACELET) is False:
                            break
                            

                        self.click_enchant()
                        image_path = self.ENCHANT_IMAGES.joinpath("in_inventory.png")
                        in_inven = imsearch.search_img_in_rect(image_path, self.win.control_panel, confidence=0.10)
                        while in_inven is None:
                            print("loop 1")
                            in_inven = imsearch.search_img_in_rect(image_path, self.win.control_panel, confidence=0.10)
                            time.sleep(.1)
                        
                        self.mouse.move_to(self.win.inventory_slots[i].random_point())
                        self.mouse.click()
                else:
                    print("Slow enchant")
                    self.click_enchant()
                    image_path = self.ENCHANT_IMAGES.joinpath("in_inventory.png")
                    in_inven = imsearch.search_img_in_rect(image_path, self.win.control_panel, confidence=0.10)
                    while in_inven is None:
                        print("loop 2")
                        in_inven = imsearch.search_img_in_rect(image_path, self.win.control_panel, confidence=0.10)
                        time.sleep(.1)
                    self.mouse.move_to(self.win.inventory_slots[random.randrange(1, 27)].random_point())
                    while self.mouseover_text("Diamond") is False:
                        self.mouse.move_to(self.win.inventory_slots[random.randrange(1, 27)].random_point())
                    self.mouse.click()

                    while self.api_m.get_if_item_in_inv(ids.DIAMOND_BRACELET):
                        print("Still crafting")
                        time.sleep(1)
                
                    time.sleep(random.uniform(5, 25))

            # Bank
            self.bank()

            self.update_progress((time.time() - start_time) / end_time)

        self.update_progress(1)
        self.log_msg("Finished.")
        self.stop()

    def click_enchant(self):
        self.mouse.move_to(self.diamond_enchant.random_point())
        self.mouse.click()


    def bank(self):
        time.sleep(1)
        self.mouse.move_to(self.get_nearest_tag(clr.YELLOW).random_point())
        self.mouse.click()
        time.sleep(1)
        while ocr.find_text("Bank", self.win.game_view, ocr.PLAIN_12, clr.ORANGE) is False:
            print("loop 4")
            time.sleep(.1)
        time.sleep(random.uniform(0.8, 1.2))
        self.verify_mouse_position(self.win.inventory_slots[random.randrange(1, 27)], "Aby")
        self.mouse.click()
        while self.api_m.get_if_item_in_inv(ids.ABYSSAL_BRACELET5):
            print("loop 5")
            time.sleep(.1)
        self.mouse.move_to(self.win.bank_slots[0].random_point())
        self.mouse.click()
        while self.api_m.get_if_item_in_inv(ids.DIAMOND_BRACELET) is False:
            print("loop 6")
            time.sleep(.1)
        self.mouse.move_to(self.win.close_bank_button.random_point())
        self.mouse.click()
        while ocr.find_text("Bank", self.win.game_view, ocr.PLAIN_12, clr.ORANGE):
            print("loop 7")
            time.sleep(.1)
        time.sleep(random.uniform(0.6, 1.2))

    def verify_mouse_position(self, rectangle, overtext):
        self.mouse.move_to(rectangle.random_point())
        while self.mouseover_text(overtext) is False:
            self.mouse.move_to(rectangle.random_point())